...
async function assertCopyIsSafe(templateRoot: string, destRoot: string, utilityName: string) {
...
async function copyUtility(templateRoot: string, destRoot: string, utilityName: string) {
  const { from, to } = await assertCopyIsSafe(templateRoot, destRoot, utilityName);
...
export async function addToClient(command: string) {
  ...
  for (const name of utilityNames) {
    await assertCopyIsSafe(templateRoot, destRoot, name);
  }
