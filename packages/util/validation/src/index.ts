function formatIsValidEmail (email: string | undefined): boolean {
  if (!email) {
    return false;
  }
...
function checkIsValidCpf (cpf: string | undefined): boolean {
  if (!cpf) return false;
...
function checkIsValidCnpj (cnpj: string | undefined): boolean {
...
function checkIsValidCpfCnpj (cpfCnpj: string | undefined): boolean {
...
function checkIsValidJSON(json: string) {
  try {
    JSON.parse(json);
    return true;
  } catch {
...
function checkIsNumber (value: unknown): boolean {
  return typeof value === 'number';
}

export const validation = {
  format: {
    email: formatIsValidEmail,
  },
  check: {
    cpf: checkIsValidCpf,
    cnpj: checkIsValidCnpj,
    cpfCnpj: checkIsValidCpfCnpj,
    json: checkIsValidJSON,
    number: checkIsNumber,
  }
}